...
SLIST_HEAD(slist_genmalloc_memory_head, genmalloc_memory_entry)
genmalloc_memory_head = SLIST_HEAD_INITIALIZER(genmalloc_memory_head);
...
void *genvector_p(const char *name, int inum, size_t elsize, const char *file,
                  const int line) {
  void *out;
  size_t mem_size;
...
void **genmatrix_p(const char *name, int jnum, int inum, size_t elsize,
...
void ***gentrimatrix_p(const char *name, int knum, int jnum, int inum,
                       size_t elsize, const char *file, const int line) {
  void ***out;
  size_t mem_size;
  char ptr_name[40];
  sprintf((char *)ptr_name, "%s ptr", name);
...
void *genmalloc_memory_add_p(void *malloc_mem_ptr, const char *name,
                             size_t size, const char *file, const int line) {
  if (SLIST_EMPTY(&genmalloc_memory_head))
  ...
  SLIST_INSERT_HEAD(&genmalloc_memory_head, genmalloc_memory_item,
                    genmalloc_memory_entries);

  return (malloc_mem_ptr);
}

void genmalloc_memory_remove_p(void *malloc_mem_ptr, const char *file,
                               const int line) {
  SLIST_FOREACH(genmalloc_memory_item, &genmalloc_memory_head,
                genmalloc_memory_entries) {
...
void genmalloc_MB_memory_report_p(const char *file, const int line) {
  SLIST_FOREACH(genmalloc_memory_item, &genmalloc_memory_head,
    ...
    printf("GENMALLOC REPORT: %20s \t%12lu Bytes, %7lu KB\n",
           genmalloc_memory_item->name, genmalloc_memory_item->mem_size,
           genmalloc_memory_item->mem_size / 1000);
  }
}
...
void genmalloc_MiB_memory_report_p(const char *file, const int line) {
  SLIST_FOREACH(genmalloc_memory_item, &genmalloc_memory_head,
    ...
    printf("GENMALLOC REPORT: %20s \t%12lu Bytes, %7lu KiB\n",
           genmalloc_memory_item->name, genmalloc_memory_item->mem_size,
           genmalloc_memory_item->mem_size / 1024);
  }
}
...
void genmalloc_MiB_memory_total_p(const char *file, const int line) {
  ...
  printf("GENMALLOC TOTAL:          Memory Size: \t%12lu Bytes, %7lu KiB, %5lu "
         "MiB\n",
...
void genmalloc_MB_memory_total_p(const char *file, const int line) {
  ...
  printf("GENMALLOC TOTAL:          Memory Size: \t%12lu Bytes, %7lu KB, %5lu "
         "MB\n",
