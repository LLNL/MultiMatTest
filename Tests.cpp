...
#pragma omp parallel for reduction(+:nmixlength,mixed_cell_count,pure_cell_count)
  ...
  float mixed_cell_fraction = (float)mixed_cell_count / ncells;
  float pure_cell_fraction = (float)pure_cell_count / ncells;
  int nmats_ave = (float)(pure_cell_count + nmixlength) / ncells;
...
#pragma omp parallel for
      for (int c = 0; c < ncellsmat[m]; c++) { // Note that this is c not C
        ...
        Density[C] += Densityfrac[m][c] * Volfrac[m][c] / Vol[C];
      }
    }
...
#pragma omp simd
        ...
        int nnm = 0; // number of nbrs with this material
        double res = MatDensity_average[m][C];
#pragma omp simd reduction(+:nnm,res)
        for (int n = 0; n < nn; n++) {
          int C_j = cnbrs[n];
          int c_j = mesh2subset[m][C_j];
          if (c_j >= 0) {
            res += Densityfrac[m][c_j] / dsqr[n];
            nnm++;
          }
        }
        MatDensity_average[m][C] = res/(double)nnm;
      }
    }
